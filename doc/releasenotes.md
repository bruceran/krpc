...
| ------- | ---- | ------------ |  ------------ | ----- | -------- | ---- | ---------- | ------ | 
| 服务契约 | proto文件 | java接口 | 无 | java接口 | 服务描述文件 | proto文件 | idl文件 | java接口 |
...
| 客户端异步调用  | 支持,java 8 future,极其强大  | 回调 | 不支持 | 回调 | 全异步 | 回调 | 回调,接口太丑 | 回调 |      
| PUSH调用  | 支持,简洁 | 支持，但配置复杂 | 不支持 | 不支持 | 支持 | 支持，接口复杂 | 支持，但配置极复杂 | 不支持 |      
...
| 框架启动配置方式  | 类dubbo | 简洁 | 无 | 类dubbo | xml文件 | ? | ? | ? |      
| 一句话点评(个人观点)  | 简单强大 | 强 但历史负担太重 内部层次太发杂 | 内网用短连接通讯不够好 | 内部实现不行 | scala语言 | http2用在内网通讯太重 | 外围做的好，内核没做好 | 过时的框架 |      
